...
namespace SMACD.Shared.Resources
{
    ...
    public class HttpResource : Resource
    {
        ...
        public static HttpResource GenerateGet(string url, IDictionary<string, string> headers = null) =>
            new HttpResource() { Method = "GET", Url = url, Headers = headers != null ? headers : new Dictionary<string, string>(), SystemCreated = true };

        public static HttpResource GeneratePost(string url, IDictionary<string, string> fields, IDictionary<string, string> headers = null) =>
            new HttpResource() { Method = "POST", Url = url, Fields = fields, Headers = headers != null ? headers : new Dictionary<string, string>(), SystemCreated = true };

        public bool MatchesExceptQuery(HttpResource that)
        {
            if (this.UriInstance.GetComponents(UriComponents.SchemeAndServer, UriFormat.Unescaped) !=
                that.UriInstance.GetComponents(UriComponents.SchemeAndServer, UriFormat.Unescaped))
                return false;
            if (this.UriInstance.GetComponents(UriComponents.Path, UriFormat.Unescaped) !=
                that.UriInstance.GetComponents(UriComponents.Path, UriFormat.Unescaped))
                return false;
            if (!this.MethodInstance.Equals(that.MethodInstance))
                return false;
            return true;
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (!MatchesExceptQuery(that))
                return false;
            if (this.UriInstance.GetComponents(UriComponents.Query, UriFormat.Unescaped) !=
