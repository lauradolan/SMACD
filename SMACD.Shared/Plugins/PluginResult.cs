...
namespace SMACD.Shared.Plugins
{
    ...
    public abstract class PluginResult
    {
        ...
        /// <returns></returns>
        public virtual async Task SummaryRunOnce(VulnerabilitySummary summary)
        {
            await Task.Yield();
        }

        /// <summary>
        ///     Execute any tasks that run generationally; that is, until all tasks report making no changes to the summary that
        ///     generation
        /// </summary>
        /// <param name="summary">Summary document</param>
        /// <returns><c>TRUE</c> if changes were made, otherwise <c>FALSE</c></returns>
        public virtual async Task<bool> SummaryRunGenerationally(VulnerabilitySummary summary)
        {
