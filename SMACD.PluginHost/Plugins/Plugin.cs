    ...
    public abstract class Plugin
    {
        ...
        public Dictionary<string, string> Options =>
            GetType().GetProperties().Where(p => p.GetCustomAttribute<ConfigurableAttribute>() != null)
                .ToDictionary(info => info.Name, info => (string)info.GetValue(this));
                ...
                if (_pluginDescription != null)
                {
                    if (Resources.Any())
                        Logger = Global.LogFactory.CreateLogger(
                            _pluginDescription.Identifier + "@" + string.Join(",", Resources.Select(r => r.ResourceId)));
                    else
        ...
        public PluginWorkingDirectory WorkingDirectory { get; private set; }
            ...
            foreach (var pair in options)
            {
                ...
                if (propertyIsConfigurable != null)
                {
                    ...
                    propertyIsConfigurable.SetValue(this,
                        Convert.ChangeType(pair.Value, (Type)propertyIsConfigurable.PropertyType));
                }
                else
                    Logger.LogWarning("Specified option '{0}' which is not understood by plugin '{1}'", pair.Key, PluginDescription.Identifier);
            }

            foreach (var property in propertyTargets.Where(t => typeof(Resource).IsAssignableFrom(t.PropertyType)))
            {
                Logger.LogDebug("Found Resource eligible target {0} with discrete Type {1}", property.Name, property.PropertyType);
        ...
        /// <summary>
        /// Get a blank scoring template which will automatically bind this scoring to the most recent Attack Tool for this Resource
        ///
        ...
        {
            var recent = WorkingDirectory.ParentResource.PluginChain.LastOrDefault(config =>
            ...
            return new ScoredResult()
            {
                ...
                {
                    Identifier = this.PluginDescription.Identifier,
                    Options = this.Options,
                    ResourceIds = this.Resources.Select(r => r.ResourceId).ToList()
                }
            };
        }
        ...
        /// <returns></returns>
        protected ScoredResult CreateBlankScoredResult(PluginDescription plugin, Dictionary<string, string> options, IList<Resource> resources)
        {
            return new ScoredResult()
            {
