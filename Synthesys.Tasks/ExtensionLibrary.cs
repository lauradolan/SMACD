...
namespace Synthesys.Tasks
{
    ...
    public class ExtensionLibrary
    {
        ...
        private readonly Dictionary<TriggerDescriptor, List<Type>> _reactionExtensions =
            new Dictionary<TriggerDescriptor, List<Type>>();
        ...
        /// </summary>
        public ReadOnlyDictionary<string, Type> ActionExtensions =>
            new ReadOnlyDictionary<string, Type>(_actionExtensions);

        /// <summary>
        ///     ReactionExtensions grouped by Trigger requirement
        /// </summary>
        public ReadOnlyDictionary<TriggerDescriptor, List<Type>> ReactionExtensions =>
            new ReadOnlyDictionary<TriggerDescriptor, List<Type>>(_reactionExtensions);
        ...
        private void PopulatePlugins()
        {
            ...
            foreach (Type extension in allExtensions)
            {
                ...
                {
                    Logger.LogCritical("Extension class {0} is not decorated with an [Extension] attribute",
                        extension.Name);
                    continue;
                }
                ...
                {
                    Logger.LogCritical(
                        "Extension {0} signals that the environment is incompatible with the extension; see logs for more information",
                ...
                {
                    Logger.LogCritical(
                        "ReactionExtension {0} is not decorated with a [TriggeredBy<Artifact/Extension/SystemEvent>] attribute, so it will never be triggered",
                        metadata.ExtensionIdentifier);
                    continue;
                }
