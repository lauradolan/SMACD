...
namespace SMACD.Plugins.OwaspZap
{
    ...
    public class OwaspZapPlugin : Plugin
    {
        ...
        public override async Task<PluginResult> Execute(PluginPointerModel pointer, string workingDirectory)
        {
            ...
            Logger.LogInformation("Starting OWASP ZAP plugin against Resource {0} with working directory {1}", pointer.Resource?.ResourceId, workingDirectory);
            HttpResource httpTarget = ResourceManager.Instance.GetByPointer(pointer.Resource) as HttpResource;
            ...
            Logger.LogDebug("Using scanner script {0} (aggressive option is {1})", pyScript, GetOption(pointer.PluginParameters, "aggressive"));
