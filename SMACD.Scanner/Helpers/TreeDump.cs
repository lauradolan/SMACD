...
namespace SMACD.Scanner.Helpers
{
    public static class TreeDump
    {
        ...
        private static readonly Type[] ValueOnlyTypes = new[] { typeof(string), typeof(TimeSpan), typeof(DateTime) };
        ...
        public static void Dump(object obj, string name = "", string indent = "", bool isLast = false)
        {
            ...
            else if (ValueOnlyTypes.Contains(obj.GetType()) || obj.GetType().GetProperties().Length == 0)
            {
                ...
                if (deadSpace + renderedValue.Length + 2 > MAX_WIDTH)
                {
                    ...
                    int c = 0;
                    foreach (string str in strings.Where(s => !string.IsNullOrEmpty(s)).Select(s => s.Trim('\r', '\n', '\t', ' ')))
                    {
        ...
        private static void DumpObject(object obj, string name = "", string indent = "", bool isLast = false)
        {
            ...
            foreach (System.Reflection.PropertyInfo property in properties)
            {
                ...
                if (property.GetIndexParameters().Length > 0)
                {
                    ...
                    {
                        var childNames = (List<string>)obj.GetType().GetProperty("ChildNames").GetValue(obj);
                        foreach (var child in childNames)
                            Dump(
                                property.GetValue(obj, new object[] { Convert.ChangeType(child, property.GetIndexParameters()[0].ParameterType) }),
                                property.Name + $"[{child}]",
                                indent,
